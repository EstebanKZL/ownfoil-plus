...
def _deep_merge_settings(target, source):
    """Merge `source` into `target` in place, recursing into nested dicts. Used for
    importing a settings export: a file from an older ownfoil version, or one that's
    just missing a section, should fill in what it has without wiping out keys it
    doesn't know about - the opposite of a plain dict.update()."""
...
def import_settings_yaml(raw_text):
    """Parse and merge an exported settings.yaml's contents into the current settings.

    Raises ValueError with a person-readable message on anything that isn't a valid
    YAML mapping - never partially applies a broken import."""
    try:
        imported = yaml.safe_load(raw_text)
    except yaml.YAMLError as e:
        raise ValueError(f'not valid YAML ({e})')
    if not isinstance(imported, dict):
        raise ValueError('the file must contain a YAML mapping (key: value pairs) at the top level')
    with settings_transaction() as settings:
        _deep_merge_settings(settings, imported)
...
def _organizer_naming_signature(organizer):
    """The parts of the organizer config that affect what a file's organized path looks
    like. A change to any of these means files already organized under the old settings
    no longer match what the organizer would produce now, so they need re-evaluating -
    not just newly-added files."""
    return (
        organizer.get('clean_names'),
        organizer.get('windows_compatible'),
        organizer.get('name_region'),
        organizer.get('name_language'),
        organizer.get('templates'),
    )

def set_library_management_settings(data):
    with settings_transaction() as settings:
        mgmt = settings['library']['management']
        old_organizer = mgmt.get('organizer') or {}
        old_signature = _organizer_naming_signature(old_organizer)
        mgmt.update(data)
        new_organizer = mgmt.get('organizer') or {}
        new_signature = _organizer_naming_signature(new_organizer)
        if new_signature != old_signature:
            # Files already organized under the old settings (a different naming
            # language, clean names just turned on/off, a Windows-compatibility change,
            # or an edited template) need renaming, not just newly-added ones.
