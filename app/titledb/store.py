...
def import_organizer_names(region_file, locale):
    """Import just the (id -> name) pairs from a second locale's titledb JSON into
    ``organizer_names``, so the organizer can name files in a language independent of
    the library's display locale - typically to sidestep accents and other symbols a
    filesystem or a specific client handles poorly.

    A small in-place update against the live titles.db rather than the atomic
    rebuild-and-swap ``import_from_json`` does: it touches nothing the merge/override
    logic depends on, and the table is fully derivable from this one file, so there is
    nothing to preserve across the wipe-and-reinsert.

    Every full rebuild of titles.db (``import_from_json``) recreates this table empty
    along with the rest of the schema, so a caller must re-run this afterwards whenever
    that happens, not only when the organizer locale itself changes.
    """
    if not os.path.isfile(region_file):
        raise FileNotFoundError(f'Cannot import organizer names, missing file: {region_file}')

    with open(region_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    rows = [(str(record['id']).upper(), record['name'])
            for record in data.values()
            if isinstance(record, dict) and record.get('id') and record.get('name')]

    for attempt in range(3):
        try:
            conn = sqlite3.connect(TITLES_DB_FILE)
            try:
                conn.execute('DELETE FROM organizer_names')
                conn.executemany(
                    'INSERT OR REPLACE INTO organizer_names ("id", name) VALUES (?, ?)', rows)
                _set_meta(conn, 'organizer_names_locale', locale)
                _set_meta(conn, 'organizer_names_imported_at', _now())
                conn.commit()
            finally:
                conn.close()
            break
            ...
            if attempt == 2:
                raise
            from db import db
            db.engine.dispose()  # closes idle pooled connections that may hold the file open
            time.sleep(1)
    logger.info(f"Imported {len(rows)} organizer name(s) for locale '{locale}'.")


def get_organizer_names_locale():
    """The locale currently imported into ``organizer_names``, or None if it has never
    been populated."""
    conn = _connect_ro()
    if conn is None:
        return None
    try:
        return _get_meta(conn, 'organizer_names_locale')
    except sqlite3.Error:
        return None
    finally:
        conn.close()
...
def get_organizer_name(title_id, locale):
    """The title's name in the organizer's alternate locale, or None.

    Also returns None when ``organizer_names`` holds a different locale than requested -
    stale data from a locale the settings no longer ask for - so a caller can safely fall
    back to the normal display-language name instead of risking a wrong-language result
    while the background titledb update catches up.
    """
    conn = _connect_ro()
    if conn is None:
        return None
    try:
        if _get_meta(conn, 'organizer_names_locale') != locale:
            return None
        row = conn.execute(
            'SELECT name FROM organizer_names WHERE "id" = ?', (title_id,)).fetchone()
