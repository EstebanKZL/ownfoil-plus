...
# Just (id -> name) for a second locale, kept apart from the merged `titles` table so it
# never affects display, search, or anything else - only the organizer reads it, and only
# when a naming language is configured independently of the library's display locale.
organizer_names = sa.Table(
    'organizer_names', metadata,
    sa.Column('id', sa.Text, primary_key=True),
