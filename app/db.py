...
def _library_looks_offline(library_path):
    """Whether a library root looks like it disappeared rather than got emptied on
    purpose. A real unmount usually makes the path stop existing, but a Docker bind
    mount to a host directory commonly keeps the directory itself around - just empty -
    once the host disconnects the drive backing it. Either shape reads the same way
    here: nothing to see, which is indistinguishable from "the drive is gone" without
    this check, and treating it as such is what avoids mass-deleting a whole library's
    tracked files just because the disk was briefly unplugged."""
    if not os.path.isdir(library_path):
        return True
    try:
        return next(os.scandir(library_path), None) is None
    except OSError:
        return True
...
def remove_missing_files_from_db():
    ...
    by_library = {}
    for f in all_files:
        by_library.setdefault(f.library_id, []).append(f)

    missing = []
    for library_id, files in by_library.items():
        library = db.session.get(Libraries, library_id)
        library_path = library.path if library else None
        if library_path and _library_looks_offline(library_path):
            logger.warning(
                f"Library '{library_path}' looks offline (path missing or empty) - "
                f"skipping removal of its {len(files)} tracked file(s) to avoid losing "
                "their verification/identification state. Reconnect it and scan again, "
