...
def resolve_duplicate_file_groups(*, ctx: GraphQLContext, info) -> "List[DuplicateFileGroup]":
    """Apps with more than one physical file attached. Admin only, like `files`
    itself - deliberately a plain ORM query rather than the selective-hydration
    machinery the rest of this module uses, since duplicate groups are normally few,
    not library-sized."""
    if not ctx.can_admin:
        return []
    import titles as titles_lib
    from library import duplicate_file_groups as _duplicate_file_groups
    from .filters import AppType
    from .types import DuplicateFileGroup

    groups = []
    for app, files in _duplicate_file_groups():
        title_info = titles_lib.get_game_info(app.title.title_id) or {}
        if app.app_type == 'DLC':
            app_name = (titles_lib.get_game_info(app.app_id) or {}).get('name')
        else:
            app_name = title_info.get('name')
        groups.append(DuplicateFileGroup(
            ...
            app_name=app_name,
            files=[_build_file(f, include_filepath=True) for f in files],
        ))
    return groups
