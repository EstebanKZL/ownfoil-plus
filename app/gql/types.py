...
class DuplicateFileGroup:
    """Files that are all attached to the same app - typically two or more physical
    copies of the same content, most often from re-downloading something already
    owned. Admin only, like `File` itself. `resolveDuplicateFiles` acts on one."""
    title_id: str = desc("The title this app belongs to.")
    title_name: Optional[str] = desc(
        "The title's display name, if titledb has one.", default=None)
    app_id: str = desc("The app id every file in this group is attached to.")
    app_type: AppType = desc("Base, Update, or DLC.")
    app_name: Optional[str] = desc(
        "The app's own display name - the DLC's name, for a DLC, or the title's own "
        "name for a base game or update.", default=None)
    files: List[File] = desc("Every physical file currently attached to this app.")
