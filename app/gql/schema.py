...
class Query:
    ...
    def duplicate_file_groups(self, info: Info) -> List[DuplicateFileGroup]:
        """Apps with more than one physical file currently attached - typically two
        or more copies of the same content, most often from re-downloading something
        already owned. Admin only; empty for any other role. See the
        `resolveDuplicateFiles` mutation to act on one."""
        return resolve_duplicate_file_groups(ctx=info.context, info=info)
