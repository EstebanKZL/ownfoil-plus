...
def test_verify_library_enqueues_the_verify_phase_directly(library):
    """Distinct from scanLibrary: no library path is involved, and it must not also
    trigger a new-file discovery/organize pass - just the verify phase, for resuming a
    previously paused/cancelled run or checking on demand."""
    data = mutate(library, """
        mutation { verifyLibrary { id taskName status } }""")

    assert data["verifyLibrary"]["taskName"] == "verify_library"
    assert data["verifyLibrary"]["status"] == "PENDING"
