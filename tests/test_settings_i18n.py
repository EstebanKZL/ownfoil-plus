...
def test_web_clean_record_checkbox_no_longer_exists(client):
    """Removed: the clean/delete-record actions on Stats' library detail view are
    always available to an admin now, no separate toggle."""
    resp = client.get("/admin/settings")
    html = resp.get_data(as_text=True)
    assert 'id="webCleanRecordEnabledCheck"' not in html
    assert "Enable cleaning records from Stats" not in html
