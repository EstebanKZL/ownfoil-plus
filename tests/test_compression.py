...
def _sweep_organize(env, **settings):
    """Run process_library_task (organize phase) with its fan-out captured, returning
    the file_ids it drove."""
    enqueued = []
    if settings:
    ...
    assert all(name == "process_file_organize" for name, _ in enqueued)
    return sorted(fid for _, fid in enqueued)


def _sweep_verify(env, **settings):
    """Run verify_library_task (verify phase) with its fan-out captured, returning the
    file_ids it drove."""
    enqueued = []
    if settings:
        env.monkeypatch.setattr(tasks, "get_settings", lambda: _settings(**settings))
    env.monkeypatch.setattr(tasks, "enqueue_or_child",
                            lambda name, data=None: enqueued.append((name, data["file_id"])))
    env.monkeypatch.setattr(tasks, "set_waiting_for_children", lambda: None)
    tasks.verify_library_task()
    assert all(name == "process_file_verify" for name, _ in enqueued)
    return sorted(fid for _, fid in enqueued)


def test_process_library_organize_phase_selects_files_with_pending_work(env):
    """process_library (phase 1) only sweeps in identify/organize work - compression is
    the verify phase's job now (see test_verify_library_phase_*), so a whole-library
    pass finishes moving files before it starts checking/compressing them."""
    ok1 = env.seed("A.nsp", identified=False)  # needs identifying
    ok2 = env.seed("B.xci")
    ok2.organized = False
    env.seed("C.nsz", compressed=True, identified=True).organized = True  # nothing left: excluded
    db.session.commit()

    assert _sweep_organize(env, organizer=True) == sorted([ok1.id, ok2.id])


def test_process_library_organize_phase_ignores_compression_only_work(env):
    """A file needing only compression must not be swept in by the organize phase - it
    surfaces later, once the organize phase settles and verify_library runs."""
    env.seed("A.nsp")  # identified, no organizer enabled: nothing for phase 1 to do

    assert _sweep_organize(env, organizer=False, compress_files=True) == []


def test_verify_library_phase_selects_files_with_pending_work(env):
    ok1 = env.seed("A.nsp")
    ok2 = env.seed("B.xci")
    env.seed("C.nsz", compressed=True)  # nothing left to do: excluded

    assert _sweep_verify(env, compress_files=True) == sorted([ok1.id, ok2.id])


def test_verify_library_phase_still_picks_up_a_file_that_also_needs_organizing(env):
    """Verify's own need (`_needs_verify`) has no dependency on the organize flag, so a
    file that hasn't been organized yet but does need verifying is still swept in by
    this phase - the split only changes *when* organizing happens, not whether
    verification still applies to a file regardless of its organize status."""
    env.monkeypatch.setattr(tasks.titles_lib.Keys, "keys_loaded", True, raising=False)
    f = env.seed("A.nsp")
    f.organized = False
    db.session.commit()

    assert _sweep_verify(env, organizer=True, verify=True, depth="signature") == [f.id]


def test_process_library_skips_a_settled_library(env):
    """With every stage disabled or satisfied, both phases enqueue nothing at all."""
    f = env.seed("C.nsz", compressed=True)
    f.organized = True
    db.session.commit()
    env.seed("A.nsp")   # only the compress stage would apply, and it is off

    assert _sweep_organize(env, compress_files=False) == []
    assert _sweep_verify(env, compress_files=False) == []
...
def test_process_file_runs_inline_stages_then_delegates(env):
    ...
    # The delegated compress_file stage now goes through enqueue_or_child (so a
    # library-wide caller can properly park and wait on it - see _drive_file); outside
    # a real running task it falls through to enqueue_task, but a direct call here has
    # no task context to fall through from, so it's mocked the same way in its own right.
    env.monkeypatch.setattr(tasks, "enqueue_or_child",
                            lambda name, data=None, **k: enqueued.append((name, data)))
    env.monkeypatch.setattr(tasks, "set_waiting_for_children", lambda: None)
...
def test_process_file_skips_a_stage_that_did_not_advance(env):
    ...
    env.monkeypatch.setattr(tasks, "enqueue_task", lambda name, data=None, **k: enqueued.append(name))
    env.monkeypatch.setattr(tasks, "enqueue_or_child", lambda name, data=None, **k: enqueued.append(name))
    env.monkeypatch.setattr(tasks, "set_waiting_for_children", lambda: None)
    f = env.seed("Game.nsp")
...
def test_process_library_continuation_starts_the_verify_phase(env):
    enqueued = []
    env.monkeypatch.setattr(tasks, "enqueue_task", lambda name, data=None, **k: enqueued.append(name))
    tasks._process_library_organize_done()
    assert enqueued == ["library_maintenance", "update_titles", "verify_library"]
