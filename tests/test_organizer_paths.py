...
from utils import clean_display_name, sanitize_filename, sanitized_path_parts, truncate_path_parts
...
CLEAN_NAME_CASES = [
    ...
    ('Super Game®', 'Super Game'),
    ('Super Game©', 'Super Game'),
    ('Super Game™: Deluxe Edition', 'Super Game Deluxe Edition'),
    ...
    ('Super Game™+', 'Super Game'),
    # Fullwidth colon, with and without surrounding spaces - removed, not normalized,
    # since sanitize_filename would otherwise turn a kept plain ':' right back into '：'.
    ('Game：Subtitle', 'Game Subtitle'),
    ('Game ：Subtitle', 'Game Subtitle'),
    ('Game： Subtitle', 'Game Subtitle'),
    ('Xenoblade Chronicles X： Definitive Edition', 'Xenoblade Chronicles X Definitive Edition'),
    # Plain colon too, for the same reason.
    ('Game: Subtitle', 'Game Subtitle'),
    # Fullwidth and plain slash
    ('FINAL FANTASY X／X-2 HD Remaster', 'FINAL FANTASY X X-2 HD Remaster'),
    ('Cave Story+/Deluxe', 'Cave Story Deluxe'),
    # All of them together
    ('Super Game™：Deluxe+ Edition®／DX', 'Super Game Deluxe Edition DX'),
]


@pytest.mark.parametrize('name, expected', CLEAN_NAME_CASES)
def test_clean_display_name(name, expected):
    assert clean_display_name(name) == expected


def test_clean_names_survives_windows_compatible_sanitization():
    """A colon or slash removed by clean_display_name must not come back once
    sanitize_filename runs afterward - that round trip was the original bug."""
    cleaned = clean_display_name('Xenoblade Chronicles X： Definitive Edition')
    assert '：' not in sanitize_filename(cleaned, windows_compatible=True)
    assert ':' not in sanitize_filename(cleaned, windows_compatible=True)
